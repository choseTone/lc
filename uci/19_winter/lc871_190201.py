...
The car starts with an infinite tank of gas, which initially has startFuel liters of fuel in it. It uses 1 liter of gas 
per 1 mile that it drives.
...
class Solution:
    def minRefuelStops(self, target, tank, stations):
        # store previous gas station, if later gas used up, pop the largest previous station
        pq = []
        stations.append((target, float('inf')))
        cnt = ploc = 0
        for loc, gas in stations:
            tank -= loc - ploc  # drive from previous station to current one
            while pq and tank < 0:
                tank -= heapq.heappop(pq)   # add gas from stored station
                cnt += 1
            if tank < 0: return -1
...
if __name__ == '__main__':
    sol = Solution()
    argv = 100, 10, [[10,60],[20,30],[30,30],[60,40]]
    ans = sol.minRefuelStops(*argv)
    print('Input : %s\nOutput: %s' % (argv, ans))
