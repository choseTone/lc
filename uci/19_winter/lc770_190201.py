...
note that variables never have a leading coefficient or unary operator like "2x" or "-x".
...
evalvars = ["c"], evalints = [2]
Output: ['-1*a*a*b*b', '1*a*b*b*b', '6*a*a*b', '-4*a*b*b', '-1*b*b*b', '-9*a*a', '-3*a*b', '6*b*b', '18*a', '-6*b', '-8']
...
class Solution:
    def basicCalculatorIV(self, expression, evalvars, evalints):
        # good case to learn python [class, default operator, regular expression, eval()]
        class Unit(collections.Counter):
        ...
        def u(s):
            s = str(val.get(s, s))
            return Unit({(s,): 1}) if s.isalpha() else Unit({(): int(s)})
        res = eval(re.sub(r'(\w+)', r'u("\1")', expression))
        return ["*".join((str(res[x]),) + x) for x in sorted(res, key=lambda x:(-len(x), x)) if res[x]]

if __name__ == '__main__':
    sol = Solution()
    argv = "((a - b) * (b - c) + (c - a)) * ((a - b) + (b - c) * (c - a))", ["c"], [2]
    ans = sol.basicCalculatorIV(*argv)
    print('Input : %s\nOutput: %s' % (argv, ans))
