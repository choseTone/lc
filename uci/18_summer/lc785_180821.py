    ...
    def isBipartite(self, graph):
        ...
        def dfs(node, g=0):
            if node in group:
                return group[node] == g
            group[node] = g
            return all(dfs(neighbor, 1-g) for neighbor in graph[node])
        return all(dfs(node) for node in range(len(graph)) if node not in group)
...
if __name__ == '__main__':
    ...
    t = time()
    ans = sol.isBipartite([[1,3], [0,2], [1,3], [0,2]])
